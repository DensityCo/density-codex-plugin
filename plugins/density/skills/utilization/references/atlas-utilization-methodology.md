...
- Route ordinary natural-language Density questions through `answer_density_question`; use narrower tools after intent and scope are clear.
- Resolve ambiguity before analytical work. A clarification performs no local query, benchmark request, chart rendering, or artifact write.
- When the user explicitly delegates "pick any" selection, choose only a live, measured, and past go-live scope. Otherwise ask which eligible building or floor to use.
- Measure full-wall latency across routing, the CLI question, optional benchmark work, SVG/HTML generation, and PNG rendering. Phase timings are diagnostics, not the caller's end-to-end time.
- Answer ordinary historical questions immediately from the current local snapshot and identify that snapshot as local historical data, never live data.
- Treat a completed all-spaces metrics snapshot as stale only when it is older than 24 hours; exactly 24 hours is fresh. For a stale eligible snapshot, start at most one organization-deduplicated background metrics refresh while preserving the current answer if launch or refresh fails.
...
- Keep per-question freshness refresh separate from onboarding deeper-history background sync. Do not put lock, worker-process, cache-path, or fingerprint mechanics in ordinary user prose.
...
When available, use the organization-scoped prepared metrics cache projected from canonical local inputs. Validate its dependency fingerprint, schema, organization, nonempty row count, and date bounds. A cache miss falls back to canonical local views with the same answer semantics; do not narrow the question or change the metric because the cache is unavailable.
...
Query non-live buildings only for explicit lifecycle, inventory, setup, or data-health questions. Ordinary utilization analysis requires a live, measured, and past go-live scope; uncertain go-live is eligible only when local lifecycle evidence explicitly confirms it is past go-live.
